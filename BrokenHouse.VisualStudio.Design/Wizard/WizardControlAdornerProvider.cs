    ...
    internal class WizardControlAdornerProvider : PrimarySelectionAdornerProvider
    {
        ...
        protected override void Activate(ModelItem item, DependencyObject view)
        {
            ...
            else if ((item.Parent != null) && item.ItemType.IsSubclassOf(typeof(WizardPage)) && item.Parent.ItemType.IsSubclassOf(typeof(WizardControl)))
            {
                ...
                WizardControl wizardControl = item.Parent.View as WizardControl;
                DesignerView  designerView  = DesignerView.GetDesignerView(wizardPage);
                ...
                if (wizardPage.EnumerateAncestors().Count() == 1)
                {
